...
# 当字典中的key不存在但被查找时，返回默认值，而不是keyError
...
class DatasetTemplate(torch_data.Dataset):
    ...
    @property
    def mode(self):
        """@property 可以让对象像访问属性一样区访问方法 self.mode，查看工作模式是训练还是测试"""
    ...
    def __setstate__(self, d): # 根据字典d更新类的属性值
        self.__dict__.update(d)

    def generate_prediction_dicts(self, batch_dict, pred_dicts, class_names, output_path=None):
        #主要目的是生成预测结果的字典，这个字典包含了预测的边界框、得分、标签等信息
        ...
        def get_template_prediction(num_samples):
            #定义内部函数，用于生成一个模板预测字典，包括名称、得分、边界框和预测标签
